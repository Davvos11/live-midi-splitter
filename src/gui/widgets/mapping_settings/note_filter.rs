...
pub fn note_filter_settings(ui: &mut Ui, settings: &mut impl CommonSettings, unique_id: String) {
    ui.horizontal(|ui| {
        ...
        ui.checkbox(
            &mut settings.transpose_mut().ignore_global,
            RichText::new("Ignore global").small(),
        )
    });
            ...
            channel_map
                ...
                .for_each(|(i, (ch_in, ch_out))| {
                    body.row(20.0, |mut row| {
                        ...
                        let is_duplicate =
                            cc_map_c.iter().filter(|&(e_ch, _)| e_ch == ch_in).count() > 1;
                        ...
                        row.col(|ui| {
                            ui.horizontal(|ui| {
                                ...
                                if let ChannelMapping::Channel(cc) = ch_out {
                                    ui.add(DragValue::new(cc).speed(0.3).clamp_range(0..=16));
                                }
                            });
                        });
                    });
